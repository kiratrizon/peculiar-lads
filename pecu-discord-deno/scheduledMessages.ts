...
// Sends every scheduled message that's due and rolls its next_run_at forward.
// Called straight from the cron (scheduler.ts) - the web app and the bot share
// one process now, so this no longer goes through an HTTP endpoint to reach
// the Discord side.
export const dispatchDueScheduledMessages = async (): Promise<number> => {
