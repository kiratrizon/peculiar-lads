...
import {
  ...
  Hash,
  Validator,
} from "Illuminate/Support/Facades/index.ts";
...
class AdminController extends Controller {
  ...
  public login: HttpDispatch = async ({ request, Auth }) => {
    if (request.method === "POST") {
      ...
      if (
        await Auth.guard("admin").attempt(credentials, credentials.remember)
      ) {
  ...
  public index: HttpDispatch = async ({ request }) => {
    ...
    for (const eventId of eventIds) {
      ...
      const eventRead = await EventRead.where("event_id", eventId)
    ...
    for (const recruitId of recruitIds) {
      ...
      const recruitRead = await RecruitRead.where("recruit_id", recruitId)
  ...
  public settings: HttpDispatch = async ({ request }) => {
    ...
    if (request.method === "POST") {
      ...
      const duplicate = await Admin.where("email", credentials.email)
        // @ts-ignore //
        .where("id", "!=", admin.id)
        .first();
      ...
      if (password) {
        if (
          !Hash.check(credentials.current_password, admin.getAuthPassword())
        ) {
          return redirect()
            .back()
            .withErrors({
