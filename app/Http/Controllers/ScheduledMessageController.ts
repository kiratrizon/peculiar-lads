...
class ScheduledMessageController extends Controller {
  ...
  private resolveChannel = async (channelId: string) => {
  ...
  private fetchRoles = async (guildId: string) => {
  ...
  private computeNextRunAt = (input: Record<string, unknown>) => {
  ...
  public index: HttpDispatch = async ({ request }, { discord_channel_id }) => {
    const channel = await this.resolveChannel(discord_channel_id as string);
    ...
    return view("admin.schedule.message", {
      ...
      channel,
      schedules: schedules.map((schedule) => this.presentSchedule(schedule)),
      roles,
      editing: null,
    });
  };
  ...
  public store: HttpDispatch = async ({ request }, { discord_channel_id }) => {
    ...
    const next_run_at = this.computeNextRunAt(data);
  ...
  public edit: HttpDispatch = async (
    ...
    return view("admin.schedule.message", {
      ...
      schedules: schedules.map((schedule) => this.presentSchedule(schedule)),
      roles,
      editing: this.presentSchedule(editing),
    });
  };
